...
`params` 完备字段：`rounds`, `depth`, `max_leaves`, `lr`, `lambda`, `min_child_weight`, `gamma`, `max_bin`, `subsample`, `colsample`, `tree_method`, `seed`, `eval_metric`；以及按需 `num_class`, `ndcg_k`, `early_stop`, `cv_folds`。
...
leaves train --data PATH [必需]
  ...
  --save-best             (默认 true：早停后截断到 best_round 再存盘；false=保留 final-round)
  --seed N                (默认 42)
...
- **早停定稿（默认）**：`--early-stop` + `--save-best`（默认 true）时，磁盘模型为 **best_round** 轮，metrics 含 `best_round` / `stopped_round` / `model_round`（`model_round == best_round`）。研究 final 轨迹时用 `--save-best=false`。
...
```powershell
# 发布后由用户 CI 推送（示例，非 leaves 内置）
# aws s3 cp release/v1 s3://models/my-task/v1 --recursive
