...
// SelectBackend 按 BackendAuto 2.1 决策表选择引擎（见 docs/backend-auto.md）。
...
func SelectBackendExplained(caps ModelCaps, hint WorkloadHint) BackendDecision {
	...
	// —— WASM：一律 Native（REV-04）——
	// GOOS=js 上 BornEngine 委托 NativeEngine（born_js.go），选 BornCPU 不会换 walk。
	// GPU-O3 的 1.6–2.6× 与 stub 冲突，作废；规则码 wasm_native（任意 batch）。
	if hint.Target == DeployWASM {
		...
		return BackendDecision{
			Backend: BackendNative,
			Rule:    "wasm_native",
			Reason: fmt.Sprintf("WASM batch=%d → Native（js 上 BornEngine 即 Native 委托，无独立张量 walk）",
				batch),
		}
	}
